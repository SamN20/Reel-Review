...
export function MovieStagingScreen({ tmdbId, onBack }: { tmdbId: number, onBack: () => void }) {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4 sm:px-0 text-white">
      <button onClick={onBack} className="text-zinc-400 hover:text-white mb-6 flex items-center gap-2 text-sm sm:text-base">
        &larr; Back to Search
      </button>
      ...
      <div className="space-y-12">
        <section>
          <h3 className="text-xl sm:text-2xl font-bold mb-6">Select Primary Poster</h3>
          <div className="flex overflow-x-auto gap-4 py-4 sm:py-6 px-2 -mx-2 hide-scrollbar">
            {data.images.posters.map((p: any) => (
              <img 
                key={p.file_path}
                src={`https://image.tmdb.org/t/p/w500${p.file_path}`} 
                className={`h-48 sm:h-64 object-cover rounded-lg cursor-pointer border-2 transition-all ${selectedPoster === p.file_path ? 'border-red-500 scale-105' : 'border-transparent opacity-60 hover:opacity-100'}`}
        ...
        <section>
          <h3 className="text-xl sm:text-2xl font-bold mb-6">Select Primary Backdrop</h3>
          <div className="flex overflow-x-auto gap-4 py-4 sm:py-6 px-2 -mx-2 hide-scrollbar">
            {data.images.backdrops.map((b: any) => (
              <img 
                key={b.file_path}
                src={`https://image.tmdb.org/t/p/w780${b.file_path}`} 
                className={`h-32 sm:h-40 object-cover rounded-lg cursor-pointer border-2 transition-all ${selectedBackdrop === b.file_path ? 'border-red-500 scale-105' : 'border-transparent opacity-60 hover:opacity-100'}`}
