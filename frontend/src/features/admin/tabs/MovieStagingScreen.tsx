...
export function MovieStagingScreen({
  ...
  const posterScroll = useDraggableScroll<HTMLDivElement>();
  const backdropScroll = useDraggableScroll<HTMLDivElement>();
    ...
    const fetchDetails = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/api/v1/admin/tmdb/movie/${tmdbId}`,
          {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4 sm:px-0 text-white">
      <button
        onClick={onBack}
        className="text-zinc-400 hover:text-white mb-6 flex items-center gap-2 text-sm sm:text-base"
      >
        &larr; Back to Search
      </button>
