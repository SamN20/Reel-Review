...
export function UsersTab() {
  ...
  const handleUpdateStatus = async (userId: number, field: 'is_active' | 'is_admin', value: boolean) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`${API_URL}/api/v1/admin/users/${userId}`, {
        [field]: value
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
                ...
                filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-zinc-800/30 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {user.email && <div className="text-zinc-500 text-xs mt-0.5">{user.email}</div>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {new Date(user.created_at).toLocaleDateString()}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${user.is_active ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-zinc-500/10 text-zinc-400 border border-zinc-500/20'}`}>
                        {user.is_active ? 'Active' : 'Suspended'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${user.is_admin ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-xs">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => handleUpdateStatus(user.id, 'is_admin', !user.is_admin)}
                        ...
                        <button
                          onClick={() => handleUpdateStatus(user.id, 'is_active', !user.is_active)}
